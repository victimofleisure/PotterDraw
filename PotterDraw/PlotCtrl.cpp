...
/*
		...
		09		22may17	in FitToData, handle invariant samples
		10		10jun17	add scroll and zoom
...
bool CPlotCtrl::FitToData()
{
	...
	if (RangeX.IsEmpty()) {	// if horizontal range is empty
		double	r = round(RangeX.Start);
		RangeX.Start = r - 1;	// spoof range
		RangeX.End = r + 1;
	}
	if (RangeY.IsEmpty()) {	// if vertical range is empty
		double	r = round(RangeY.Start);
		RangeY.Start = r - 1;	// spoof range
		RangeY.End = r + 1;
	}
...
void CPlotCtrl::BeginDrag(CPoint point)
{
	SetCapture();
	m_ptDrag = point;
	m_bIsDragging = true;
...
void CPlotCtrl::SetDragCursor()
{
	LPCTSTR	szCursor;
	switch (m_nDragMask & AXM_ALL) {
	case AXM_HORZ:
		szCursor = IDC_SIZEWE;
		break;
	case AXM_VERT:
...
void CPlotCtrl::EndDrag()
{
	ReleaseCapture();
	m_bIsDragging = false;
}

void CPlotCtrl::ScrollRuler(int iRuler, double fDelta)
{
	CDblRange&	rng = m_RulerInfo[iRuler].m_Range;
	rng -= fDelta * rng.Length();
}

void CPlotCtrl::ZoomRuler(int iRuler, double fDelta, double fPos)
{
	CDblRange&	rng = m_RulerInfo[iRuler].m_Range;
	double	fOrigin = rng.Start + fPos * rng.Length();
	rng = (rng - fOrigin) * fDelta + fOrigin;
}
...
void CPlotCtrl::OnParentNotify(UINT message, LPARAM lParam)
{
	CWnd::OnParentNotify(message, lParam);
	switch (message) {
	case WM_LBUTTONDOWN:
		if (m_nDragMask) {	// if drag enabled
			CPoint	ptCursor;
...
void CPlotCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (m_nDragMask)	// if drag enabled
		BeginDrag(point);
...
void CPlotCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	if (m_bIsDragging)	// if drag in progress
		EndDrag();
	CWnd::OnLButtonUp(nFlags, point);
}

void CPlotCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_bIsDragging) {	// if drag in progress
		CPoint	ptDelta(point - m_ptDrag);
		m_ptDrag = point;
		bool	bScrolled = false;
		CSize	szPlot = m_PlotRect.Size();
		if ((m_nDragMask & AXM_HORZ) && ptDelta.x && szPlot.cx) {
			double	fDelta = double(ptDelta.x) / szPlot.cx;
			if (m_VisibleRulers & RM_TOP)
				ScrollRuler(RULER_TOP, fDelta);
			if (m_VisibleRulers & RM_BOTTOM)
				ScrollRuler(RULER_BOTTOM, fDelta);
			bScrolled = true;
		}
		if ((m_nDragMask & AXM_VERT) && ptDelta.y && szPlot.cy) {
			double	fDelta = double(ptDelta.y) / szPlot.cy;
			if (m_VisibleRulers & RM_LEFT)
				ScrollRuler(RULER_LEFT, fDelta);
			if (m_VisibleRulers & RM_RIGHT)
				ScrollRuler(RULER_RIGHT, fDelta);
			bScrolled = true;
		}
		if (bScrolled)
			Update();
...
BOOL CPlotCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_nDragMask) {	// if drag enabled
		CPoint	ptCursor(pt);
		ScreenToClient(&ptCursor);
		if (CRect(CPoint(0, 0), m_WndSize).PtInRect(ptCursor)) {	// if cursor within client area
			CSize	szPlot = m_PlotRect.Size();
			double	fZoomStep = 1.5;
			double	fDelta = pow(fZoomStep, double(-zDelta) / WHEEL_DELTA);
			if ((m_nDragMask & AXM_HORZ) && szPlot.cx) {
				double	x = double(ptCursor.x - m_PlotRect.left) / szPlot.cx;
				if (m_VisibleRulers & RM_TOP)
					ZoomRuler(RULER_TOP, fDelta, x);
				if (m_VisibleRulers & RM_BOTTOM)
					ZoomRuler(RULER_BOTTOM, fDelta, x);
			}
			if ((m_nDragMask & AXM_VERT) && szPlot.cy) {
				double	y = double(ptCursor.y - m_PlotRect.top) / szPlot.cy;
				if (m_VisibleRulers & RM_LEFT)
					ZoomRuler(RULER_LEFT, fDelta, y);
				if (m_VisibleRulers & RM_RIGHT)
					ZoomRuler(RULER_RIGHT, fDelta, y);
			}
			Update();
		}
	}
