...
/*
		...
		05		29jun17	add vertical font style
...
//printf("xmod=%f x1=%f x2=%f n=%f\n", xmod, x1, x2, n);
		...
		if (style & CBRS_ALIGN_LEFT) {	// if docked left
			...
			if (style & VERTICAL_FONT)	// if using vertical font
				TextX -= TextExt.cy;	// assume font rotated 90 degrees counterclockwise
			...
			for (double ry = y1; ry < y2; ry += dy) {
				...
				if (!IsMinor) {	// if major tick
					...
					if (style & VERTICAL_FONT) {	// if using vertical font
						...
						TextVCenter = ext.cx >> 1;	// vertical center varies per label
					}
					if (style & HIDE_CLIPPED_VALS) {	// if hiding clipped values
