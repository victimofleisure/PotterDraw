...
/*
		...
		08		22may17	add range definitions
		09		10jun17	add scroll and zoom
...
public:
	...
	UINT	GetDragMask() const;
	void	SetDragMask(UINT nAxisMask);
...
// Member data
	...
	bool	m_bIsDragging;			// true if dragging
	UINT	m_nDragMask;			// mask to enable dragging for each axis
	CPoint	m_ptDrag;				// previous cursor position during drag, in client coords
...
// Helpers
	...
	void	EndDrag();
	void	SetDragCursor();
	void	ScrollRuler(int iRuler, double fDelta);
	void	ZoomRuler(int iRuler, double fDelta, double fPos);
...
inline UINT CPlotCtrl::GetDragMask() const
{
	return(m_nDragMask);
}

inline void CPlotCtrl::SetDragMask(UINT nAxisMask)
{
	m_nDragMask = nAxisMask;
}
